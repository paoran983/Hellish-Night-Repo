...
public class MoveListUIController : MonoBehaviour
{
    [SerializeField] private MoveListPageUI moveList;
    [SerializeField] private MoveListPageUI actionMoveList;
    [SerializeField] private List<MoveListPageUI> moveLists;
    ...
    public void ResetMoveList()
    {
        ...
        {
            MoveListPageUI curMoveList = moveLists[i];
            if (curMoveList != null)
            {
        ...
        if (abilityDic != null)
        {
            for (int i = 0; i < abilityDic.Count; i++)
            {
                ...
                MoveListPageUI curMoveList = moveLists[i];
                AbilityItemSO abilityData = curAbility.item as AbilityItemSO;
                if (abilityData != null && curMoveList != null)
                {

                    if (abilityData.MoveList != null)
                    {
                        for (int j = 0; j < curAbility.level; j++)
                        {
                            Move curMove = abilityData.MoveList[j];
                            if (!curMove.IsEmpty())
                            {
                                curMoveList.UpdateData(j, curMove.item.GetImage(), 1);
                                curMoveList.MoveData.AddItem(curMove);
                            }
    ...
    public void Activate(bool isActive)
    {
        ...
        if (isActive)
        {
            ...
            foreach (MoveListPageUI curMoveList in moveLists)
            {
                curMoveList.Show();
